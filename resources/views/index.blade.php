...
<section>
    ...
    <div class="panel panel-default">
        <div class="panel-body">
            <form role="form" name="form_a" action="{{route('search-flight')}}" method="post" id="search">
              {{csrf_field() }}
              <!-- {{ csrf_token() }} -->
                <div class="row">
                    <div class="col-sm-4">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="from" id="from">
                                @foreach ($cities as $city)
                                    <option value="{{$city->id}}">{{$city->cityname}}  ({{ $city->shortname}})</option>
                                @endforeach
                            </select>          
...
<!-- Show/ hide when press radio -->
<script>
    $(document).ready(function(){
        $("input[value='one-way']").click(function(){
             $("#here").hide('fast/2000/slow');
         })
        $("input[value='return']").click(function(){
            $("#here").show('slow/400/fast', function() {
